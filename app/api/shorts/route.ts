...
export async function POST(req:Request){const u=await getCurrentUser();if(!u)return reply({error:'Sign in first.'},401);if(!sameOrigin(req))return reply({error:'Request origin rejected.'},403);try{const now=Date.now(),day=new Date(now).toISOString().slice(0,10);
